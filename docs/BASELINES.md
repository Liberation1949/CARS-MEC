...
- **Independent reimplementation 独立重实现**：是 / Yes（`reports/experiments/R3_bpso_rata_la_adaptation_contract.yaml`，`source_aligned_project_adapted`）。
...
- **Project adaptations 项目适配**：
  - 传输速率使用项目共享链路原语（逐链路独立无线信道，无跨链路干扰模型）；内部 fitness 的 $t_i^{off}/E_i^{trans}$ 使用 R2 共享物理原语； / transmission rate uses the project's shared link primitives (per-link independent wireless channels, no cross-link interference model); $t_i^{off}/E_i^{trans}$ in the internal fitness uses the R2 shared physical primitives;
  ...
  - Algorithm 1 精确指派循环（原文为图片）依据 Theorem 1 排序目标与正文 prose 冻结（`POSSIBLE_INTERPRETATION`）。 / the exact assignment loop of Algorithm 1 (an image in the paper) is frozen from the Theorem 1 ordering objective and the manuscript prose (`POSSIBLE_INTERPRETATION`).
...
- **Parameter / search budget 参数/搜索预算**（TEST_ONLY_NOT_FORMAL，非调参 / not tuning）：`population_size_max`、`max_iterations_max`、`particle_evaluation_cap_max`、`reliability_threshold`、`inertia_weight`、`cognitive_coefficient`、`social_coefficient`；正式配置见 `configs/r6/frozen_method_configs/bpso_frozen.yaml`。
...
- **Independent reimplementation**：是 / Yes（`R3_jtora_adaptation_contract.yaml`，`source_aligned_project_adapted`）。
...
- **Project adaptations**：
  ...
  - Algorithm 2 迭代上界 `max_outer_iterations` 封顶（原文无显式上界，`IMPLEMENTATION_SPEC_GAP`）。 / Algorithm 2 iteration bound is capped by `max_outer_iterations` (the paper has no explicit bound, `IMPLEMENTATION_SPEC_GAP`).
...
- **Independent reimplementation**：是 / Yes（`R3_NFA_adaptation_contract.yaml`，`source_aligned_project_adapted`）。
- **Source-faithful components**：
